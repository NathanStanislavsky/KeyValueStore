...
WAL::WAL(const std::string &filename) : filename(filename) {
  ...
  if (!file_stream.is_open()) {
    ...
    if (!file_stream.is_open()) {
      std::cerr << "Failed to open or create WAL file: " << filename
                << std::endl;
    }
  }
}
...
bool WAL::write(const std::string &key, const std::string &value) {
  ...
  crc = crc32_update(crc, reinterpret_cast<const uint8_t *>(key.data()),
                     key.size());
  crc = crc32_update(crc, reinterpret_cast<const uint8_t *>(value.data()),
                     value.size());

  header.checksum = crc ^ 0xFFFFFFFFu;

  file_stream.write(reinterpret_cast<const char *>(&header),
...
std::vector<std::pair<std::string, std::string>> WAL::readAll() {
  ...
  while (true) {
    ...
    // we can't read the full header of record meaning it is either EOF or
    // header was cut off due to crash
    if (!file_stream.read(reinterpret_cast<char *>(&header),
                          sizeof(WALRecordHeader))) {
      break;
    }
    ...
    recalculated_crc =
        crc32_update(recalculated_crc,
                     reinterpret_cast<const uint8_t *>(key.data()), key.size());
    recalculated_crc = crc32_update(
        recalculated_crc, reinterpret_cast<const uint8_t *>(value.data()),
        value.size());
...
WAL::readAllFromFile(const std::string &path) {
...
void WAL::rotate() {
  ...
  if (!file_stream.is_open()) {
    ...
    if (!file_stream.is_open()) {
      std::cerr << "Failed to reopen WAL file after rotation: " << filename
                << std::endl;
    }
  }
}
