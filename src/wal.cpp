...
bool WAL::write(const std::string &key, const std::string &value) {
  ...
  bool success = file_stream.good();
  if (!success) {
    std::cerr << "WAL stream error! eof=" << file_stream.eof()
              << " fail=" << file_stream.fail()
              << " bad=" << file_stream.bad() << std::endl;
    file_stream.clear();
  }
...
std::vector<std::pair<std::string, std::string>> WAL::readAll() {
  ...
  // Clear the EOF and fail bits generated by hitting the end of the file
  file_stream.clear();
